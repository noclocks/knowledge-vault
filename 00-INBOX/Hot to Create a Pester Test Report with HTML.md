...
tags:
  ...
  - Status/NA
aliases:
  - Hot to Create a Pester Test Report with HTML
publish: true
permalink:
description:
image:
...
[Pester](https://github.com/pester/Pester) is a unit testing framework built into PowerShell that allows developers of all kinds to write unit, integration and acceptance tests across a wide range of situations. It's a tool that enables developers to write code in a domain-specific language (DSL) representing various kinds of tests.

## ReportUnit

Install a tool called [ReportUnit]() which can be found on [Nuget.org]()
...
```powershell
Invoke-Pester -Path "path/to/tests.ps1" -OutputFormat  NUnitXml -OutputFile "Output/TestResults.xml" -Show None

ReportUnit.exe "Output/"
```
